...
from ..utils.transforms import HeatmapTransform
...
class GridView:
    ...
    def tile_slice(self, tile_r: int, tile_c: int):
        ...
        assert tile_r < self.tiles_r, "tile {} is out of range with max {}".format(tile_r, self.tiles_r)
        assert tile_c < self.tiles_c, "tile {} is out of range with max {}".format(tile_c, self.tiles_c)
...
class NumpyGridView(GridView):
    ...
    def shuffled(self):
        ...
        for r in range(self.tiles_r):
            for c in range(self.tiles_c):
                shuffled[self.tile_slice(r, c)] = self.img[self.tile_slice(idx_r[r, c], idx_c[r, c])]
        return shuffled
...
class Perturber:
    ...
    def get_idxes(self, hmap: np.ndarray, reverse=False) -> list:
        """ return a sorted list with shape length NUM_CELLS of which pixel/cell index to blur first """
        raise NotImplementedError
